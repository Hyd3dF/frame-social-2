...
func TestDBSlowDoesNotBlockSend(t *testing.T) {
	...
	if elapsed > 50*time.Millisecond {
		t.Fatalf("sendMessage should be <50ms with cache, got %v", elapsed)
	}
	if len(srv.pending.List("conversation:slow")) != 1 {
...
func TestPendingSurvivesForListMessages(t *testing.T) {
	...
	if !strings.Contains(w.Body.String(), "pending1") {
		t.Fatalf("pending message not merged, body %s", w.Body.String())
	}
}
