...
RETURN AFTER`, map[string]any{
	...
	// Round-trip check: never hand out an access token this instance itself
	// would reject on the next request (secret/config drift guard).
	if parsed, parseErr := security.ParseAccessToken(s.cfg.JWTSecret, access); parseErr != nil || parsed != account {
